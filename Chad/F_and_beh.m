...
for fly = 1: length(flies)
    ...
    for task = 1: length(tasks)
        ...
        % grab fluoresence data
        F = in.(flies{fly}).(tasks{task}).smooth_Zscore;
        fps = size(F,2)/ 45;
        x_F = (1:length(F))/fps; 
        ...
        for o = 1: length(odors);
            ...
            %grab F data for each odor. 
            F_mean.(tasks{task}).(odors{o})(fly,1:length(F),:) = nanmean(F(strcmp(odor_id, odors{o}), :,:));
            F_odor.(tasks{task}).(odors{o})(fly,:,:,:) = F(strcmp(odor_id, odors{o}), :,:);
            ...
            F_response_frames = find(x_F >= odor_on,1): find(x_F >= (odor_on +1),1);
            F_pre_frames = find(x_F >= odor_on-2,1): find(x_F >= odor_on-1,1);
            F_response.(tasks{task}).(odors{o}) = nanmean(F_odor.(tasks{task}).(odors{o})(fly,:,F_response_frames,:),3) - nanmean(F_odor.(tasks{task}).(odors{o})(fly,:,F_pre_frames,:),3);

            F_peak.(tasks{task}).(odors{o})(fly,:,:) = max(F_odor.(tasks{task}).(odors{o})(fly,:,F_response_frames,:),[],3) - nanmean(F_odor.(tasks{task}).(odors{o})(fly,:,F_pre_frames,:),3);
            roi_F_peak = squeeze(nanmean(F_peak.(tasks{task}).(odors{o})(fly,:,:),2));
            roi_F_response =  squeeze(nanmean(F_response.(tasks{task}).(odors{o})(fly,:,:),2));
            lap_F_peak = squeeze(nanmean(F_peak.(tasks{task}).(odors{o})(fly,:,:),3));
            lap_F_response =  squeeze(nanmean(F_response.(tasks{task}).(odors{o})(fly,:,:),3));
            ...
            figure(Fplot)
            hold on
            range = find(x_F >= odor_on - 6):find(x_F >= odor_on + 12);
            mean_F = squeeze(nanmean(nanmean(F_odor.(tasks{task}).(odors{o})(fly,:,:,:),2),4));
            plot(x_F(range)-20.3, mean_F(range,:), 'DisplayName', [tasks{task} ' ' odors{o}])
            legend

            figure(behplot)
            hold on
            range = find(x >= - 6):find(x >= 12);
            plot(x, data_mean.(tasks{task}).(odors{o})(fly,:), 'DisplayName', [tasks{task} ' ' odors{o}]);
            legend
        end
...
%     for o = 1: length(odors)
%         figure;
%         data_2_plot = delta_trial.(tasks{task}).(odors{o});
%         data_2_plot(data_2_plot== 0) = NaN;
%         if size(data_2_plot,2) > 8
%             data_2_plot = data_2_plot(:,1:8);
%         end
% 
%         trial_mean = nanmean(data_2_plot,1);
% 
%         x = 1:length(trial_mean);
% 
%         labels = arrayfun(@num2str, x, 'UniformOutput', 0);
% 
%         boxplot(data_2_plot, labels);
%         hold on
%         x = repmat(x',size(data_2_plot,1));
%         y = data_2_plot';
%         mdl = fit(x(:,1), y(:), "poly1");
%         plot(mdl)
%         plot(data_2_plot', 'Marker', 'o', 'Color', 'k');
% 
%         trial_mean = nanmean(data_2_plot,1);
%         % 
%         % trial_sem = squeeze(nanstd(lap_means.(tasks{task}).(odors{o}),[],1)./sqrt(size(lap_means.(tasks{task}).(odors{o}),1))); 
%         % 
%         % errorbar(x,trial_mean, trial_sem)
% 
%         if measure == "movspd"
%             ylabel("average speed mm/s")
%         else 
...
%         title([tasks{task} ' ' odors{o}])
% 
%         for fly = 1: size(lap_means.(tasks{task}).(odors{o}))
%             set(0,'DefaultFigureVisible','on');
% 
%             data_2_test = squeeze([lap_means.(tasks{task}).(odors{o})(fly,1,:); lap_means.(tasks{task}).(odors{o})(fly,2,:)]);
% 
%             figure; hold on
%             %compare the pre vs during data
%             [p,t,stats] = anova1(data_2_test');
%             [c,m,h,gnames] = multcompare(stats);
% 
% 
%             fly_test.(tasks{task}).(odors{o})(fly) = c(end);
% 
%         end
% 
%     end
