...
function [out] = extract_data_manual_labeling(folder, thresh);
%find current folder wd
main_folder = pwd;
%change wd to folder with data
cd(folder);

out_file = dir('extracted_manual_data.mat');
...
%% input ROI data and extract fluorescence using manual labeling of ROIs
% if ~isempty(out_file)
%     load(out_file.name);
% 
...
% else
% Get user input
prompt = {'Enter image acquisition rate (in fps)','Enter any trials to exclude (no commas)'};
dlgtitle = 'Gathering user input';
fieldsize = [1 45; 1 45];
definput = {'30',''};
answer = inputdlg(prompt,dlgtitle,fieldsize,definput);
fps = str2double(answer{1});
exclude_trials = sscanf(answer{2}, '%f');

%grab image names for any planes where rois have been manually labeled
F_name = dir('plane*ROIs.csv');
...
for plane = 1: length(n_planes)

    %load data
    temp_data = readtable([F_name(plane).name]);
    temp_data(:,1)=[];
    F = table2array(temp_data);


    %find the number of trials and rois and frames per trial
    ntrials = numel(fieldnames(params));
    rois = size(F,2);
    fpt = round(length(F)/ntrials);
    

    % smooth F
    for n = 1: rois
        F(:,n) = smooth(F(:,n),31,'sgolay',5);
    end
    
    %exclude any bad trials
    if ~isnan(exclude_trials)
        for trial = 1:size(exclude_trials,1);
            frames_2_exclude = [fpt * (exclude_trials(trial) -1) + 1: fpt * exclude_trials(trial)];
            F(frames_2_exclude,:) = NaN;
        end
    end

    %massage the F data so each trial has the same number of planes
    F_massaged = NaN(fpt * ntrials, rois);
    F_massaged(1:length(F), :) = F;

    %reshape F data to be divided into individual trials
    F_trial = reshape(F_massaged, [fpt, ntrials, rois]);
    
    %find frames where odor comes on and off
    odor_time = params.Data_1.parameter.preO;
    odor_dur = params.Data_1.parameter.dur;
    pre_odor_frames = (1:(fps * odor_time));
    
    %find the Zscore and DFF for every trial for every ROI
    baseline = nanmean(F_trial(pre_odor_frames, :, :));
    baseline_sd = std(F_trial(pre_odor_frames, :, :));
    Zscore = (F_trial- baseline) ./ baseline_sd;
    DFF = (F_trial -baseline) ./ baseline;
 
    %collect data for output
    out.F = [out.F F_trial];
    out.Zscore = [out.Zscore Zscore];
    out.DFF = [out.DFF DFF];
end


% change the wd back to the main folder
save("extracted_manual_data", "out")
cd(main_folder);
end
